...
const uint16_t  RANGE_LIMIT[]       = {148, 118, 113}; /* 80, 100, 120 */
 ...
 const int16_t   SENSITIVE_DELTA[]   = {30, 30, 15, 9, 11, 4, 4, 3, 6, 3, 3, 3, 3};
 ...
 */
 uint16_t ADC_Process_Value(uint16_t adcDistantValue) {
    ...
    static uint8_t lastDistLevel = 0;
    ...
    if (dist_level > 0)
    {
        ...
        uint8_t delta = abs((int8_t)lastDistLevel - dist_level);
        if (delta > 1)
        {
        ...
        if (countStableVal < COUNT_STABLE_SAMPLE)
        {
            countStableVal++;
            return 0xFFFF;
        }
    ...
    {
        // for special wave hand case        
//        if (lastDistLevel<=4 && countValidVal>7 && countValidVal<=COUNT_STABLE_SAMPLE)
//        {
//            countValidVal = 0;
//            return (uint16_t)lastDistLevel * 40;
//        }

        countStableVal = 0;
        countValidVal = 0;
        // BREAKS_SIZE +1 for logical order
        // and assure it will not affect when 
        // next dist_level = BREAK_SIZE -1.
        lastDistLevel = BREAKS_SIZE + 1;
    }
    return 0;
}
